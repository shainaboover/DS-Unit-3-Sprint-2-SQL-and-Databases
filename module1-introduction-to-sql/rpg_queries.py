...
query_1 = """SELECT
    count(distinct character_id) as unique_count 
...
query_2 = """SELECT
    ...
    ,COUNT(distinct mage.character_ptr_id) as mage_count
    ,COUNT(distinct thief.character_ptr_id) as thief_count
FROM charactercreator_character character
LEFT JOIN charactercreator_cleric cleric ON cleric.character_ptr_id = character.character_id
LEFT JOIN charactercreator_fighter fighter ON fighter.character_ptr_id = character.character_id
LEFT JOIN charactercreator_mage mage ON mage.character_ptr_id = character.character_id
LEFT JOIN charactercreator_thief thief ON thief.character_ptr_id = character.character_id"""
...
LEFT JOIN armory_weapon weapon ON item.item_id = weapon.item_ptr_id"""
...
query_6 = """SELECT
    cci.character_id
    ,COUNT(distinct aw.item_ptr_id) as weapons_count
FROM charactercreator_character_inventory cci
...
query_7 = """SELECT AVG(items_count) as avg_items_count
FROM (
...
query_8 = """SELECT AVG(weapons_count) as avg_weapons_count
FROM (
    SELECT cci.character_id
        ,COUNT(distinct aw.item_ptr_id) as weapons_count
    FROM charactercreator_character_inventory cci
